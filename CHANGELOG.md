...
#### Socket Core
- **`SO_REUSEPORT` guarded on all platforms** — `socket.SO_REUSEPORT` doesn't exist on Windows (missing from the
  Windows SDK) and may raise `AttributeError` on older Python builds ; wrapped with `contextlib.suppress` in both
  `ThreadingSocket` and `AsyncSocket` so the server can start on Windows 7 and other platforms without it
  ([330be08])
